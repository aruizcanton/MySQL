DECLARE
  ...
  IS
    ...
    and CONCEPT_NAME in ('USERS', 'user_certifications', 'user_badges', 'COURSES', 'course_users', 'course_units', 'GROUPS'
    , 'groups_courses', 'GROUPS_USERS', 'CATEGORIES', 'categories_courses', 'BRANCHES', 'branches_courses', 'BRANCHES_USERS'
    , 'TESTS', 'TEST_ANSWERS', 'user_progress_unit', 'SURVEY', 'SURVEYANSWERS', 'profile', 'rasgos', 'roles', 'ops', 'consumer_prefer'
    , 'warnings', 'consumption_prefer', 'formulario');
...
BEGIN
  ...
  LOOP
      ...
      UTL_FILE.put_line(fich_salida_pkg,'    @code = RETURNED_SQLSTATE, @msg = MESSAGE_TEXT, @errno = MYSQL_ERRNO;' );
      UTL_FILE.put_line(fich_salida_pkg,'    select ''Insercion en MTDT_MONITOREO erronea. inserta_monitoreo procedure error.'';' );
...
/************/
      if (reg_summary.HISTORY IS NOT NULL) then
        ...
        UTL_FILE.put_line(fich_salida_pkg,'PROCEDURE pos_' || nombre_proceso || ' (IN fch_carga_in VARCHAR(8), IN fch_datos_in VARCHAR(8), IN forzado_in VARCHAR(1))');
        UTL_FILE.put_line(fich_salida_pkg,'BEGIN' );
        UTL_FILE.put_line(fich_salida_pkg,'  DECLARE code CHAR(5) DEFAULT ''00000'';' );
        UTL_FILE.put_line(fich_salida_pkg,'  DECLARE msg TEXT;' );
        UTL_FILE.put_line(fich_salida_pkg,'  DECLARE errno INT;' );
        UTL_FILE.put_line(fich_salida_pkg,'  DECLARE finalizado INT(1) DEFAULT 0;' );
        UTL_FILE.put_line(fich_salida_pkg,'  DECLARE exis_tabla number(1);');
        UTL_FILE.put_line(fich_salida_pkg,'  DECLARE exis_partition number(1);');
        UTL_FILE.put_line(fich_salida_pkg,'  DECLARE fch_particion varchar(8);');
        ...
        UTL_FILE.put_line(fich_salida_pkg,'  table_name = ''SAH_' || reg_summary.CONCEPT_NAME || ''' and');
        ...
        UTL_FILE.put_line(fich_salida_pkg,'    if finalizado then');
        ...
        UTL_FILE.put_line(fich_salida_pkg,'  close cursor_pru;' );
        UTL_FILE.put_line(fich_salida_pkg,'' );
        ...
        UTL_FILE.put_line(fich_salida_pkg,'  else' );
        UTL_FILE.put_line(fich_salida_pkg,'    set @sql_text := concat(''ALTER TABLE '',''' || NAME_DM || ''', ''SAH_'', ''' || reg_summary.CONCEPT_NAME || ''', '' ADD PARTITION '', ''' || v_nombre_particion || ''', ''_'', fch_carga_in, ''VALUES LESS THAN ('', fch_particion , '') TABLESPACE '', ''' || TABLESPACE_SA || ''', '';'');');
        UTL_FILE.put_line(fich_salida_pkg,'    prepare stmt from @sql_text;' );
        UTL_FILE.put_line(fich_salida_pkg,'    execute stmt;' );
        UTL_FILE.put_line(fich_salida_pkg,'    DEALLOCATE PREPARE stmt;' );
