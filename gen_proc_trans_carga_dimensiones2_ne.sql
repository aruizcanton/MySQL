    ...
    then
            ...
            loop
              ...
              then
                ...
                UTL_FILE.put_line(fich_salida_pkg,'');
                UTL_FILE.put_line(fich_salida_pkg, '  DROP PROCEDURE IF EXIST ' || NAME_DM || '.nreg_' || nombre_proceso);
                UTL_FILE.put_line(fich_salida_pkg, '  PROCEDURE ' || NAME_DM || '_nreg_' || nombre_proceso || ' (IN fch_carga_in VARCHAR(8), IN fch_datos_in VARCHAR(8), IN forzado_in VARCHAR(1), OUT num_reg_afected_out int)');
                UTL_FILE.put_line(fich_salida_pkg, '  BEGIN');
                UTL_FILE.put_line(fich_salida_pkg, '    DECLARE num_filas_insertadas INT;');
                UTL_FILE.put_line(fich_salida_pkg, '    DECLARE var_fch_inicio datetime := sysdate;');
                UTL_FILE.put_line(fich_salida_pkg, '');
