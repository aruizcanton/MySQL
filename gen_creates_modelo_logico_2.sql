    ...
    LOOP
                ...
                if (UPPER(TRIM(r_mtdt_modelo_logico_COLUMNA.VDEFAULT)) = 'AUTO') then
                  ...
                  if (r_mtdt_modelo_logico_TABLA.CI = 'N' and ((r_mtdt_modelo_logico_COLUMNA.COLUMN_NAME = 'CVE_' ||  SUBSTR(r_mtdt_modelo_logico_COLUMNA.TABLE_NAME,5)) or (r_mtdt_modelo_logico_COLUMNA.COLUMN_NAME = SUBSTR(r_mtdt_modelo_logico_COLUMNA.TABLE_NAME,5) || '_ID'))) then
                    if (r_mtdt_modelo_logico_COLUMNA.NULABLE = 'N') then
                      ...
                      DBMS_OUTPUT.put_line('`' || r_mtdt_modelo_logico_COLUMNA.COLUMN_NAME || '`'|| '          ' || 'BIGINT' || r_mtdt_modelo_logico_COLUMNA.VDEFAULT || ' NOT NULL AUTO_INCREMENT');
                    else
                      ...
                      DBMS_OUTPUT.put_line('`' || r_mtdt_modelo_logico_COLUMNA.COLUMN_NAME || '`'|| '          ' || 'BIGINT' || r_mtdt_modelo_logico_COLUMNA.VDEFAULT || ' AUTO_INCREMENT');
                    end if;
                  else
            ...
            CASE
              WHEN (INSTR(r_mtdt_modelo_logico_COLUMNA.DATA_TYPE, 'NUMBER') > 0) THEN
                if (r_mtdt_modelo_logico_TABLA.CI = 'N' and ((r_mtdt_modelo_logico_COLUMNA.COLUMN_NAME = 'CVE_' ||  SUBSTR(r_mtdt_modelo_logico_COLUMNA.TABLE_NAME,5)) or (r_mtdt_modelo_logico_COLUMNA.COLUMN_NAME = SUBSTR(r_mtdt_modelo_logico_COLUMNA.TABLE_NAME,5) || '_ID'))) then
                  ...
                  if (r_mtdt_modelo_logico_COLUMNA.COLUMN_NAME = 'CVE_DIA' OR r_mtdt_modelo_logico_COLUMNA.COLUMN_NAME = 'CVE_DAY' or r_mtdt_modelo_logico_COLUMNA.COLUMN_NAME = 'DAY' or r_mtdt_modelo_logico_COLUMNA.COLUMN_NAME = 'CVE_MES' or r_mtdt_modelo_logico_COLUMNA.COLUMN_NAME = 'CVE_WEEK') then
                    v_nombre_campo_particionado:= r_mtdt_modelo_logico_COLUMNA.COLUMN_NAME;
                    if (r_mtdt_modelo_logico_COLUMNA.NULABLE = 'N') then
                      DBMS_OUTPUT.put_line('`' || r_mtdt_modelo_logico_COLUMNA.COLUMN_NAME || '`'|| '          ' || 'BIGINT' || ' NOT NULL');
                    else
        ...
        LOOP
          ...
          if v_tipo_particionado = 'S' then
            ...
            if v_tipo_particionado = 'D' then
              ...
              if lista_pk (indx) = 'CVE_WEEK' then
                v_campo_parti_en_pk := 1;
              end if;          
            else
            ...
            IF indx = lista_pk.LAST THEN
              if v_campo_parti_en_pk = 1 then
                ...
                if (v_CVE_DIA_es_col=1 or v_CVE_MES_es_col=1 or v_CVE_WEEK_es_col=1) then
                  if v_tipo_particionado = 'D' then
                    --DBMS_OUTPUT.put_line('CVE_DIA, ' || lista_pk (indx) || ') ');
                    --DBMS_OUTPUT.put_line('CVE_DAY, ' || lista_pk (indx) || ') ');
                    DBMS_OUTPUT.put_line(v_nombre_campo_particionado || ', ' || lista_pk (indx) || ') ');
                  elsif v_tipo_particionado = 'W' then
                    DBMS_OUTPUT.put_line(v_nombre_campo_particionado || ', ' || lista_pk (indx) || ') ');
                  else
      ...
      if ((regexp_count(substr(r_mtdt_modelo_logico_COLUMNA.TABLE_NAME, 1, instr(r_mtdt_modelo_logico_COLUMNA.TABLE_NAME, '_')), '^?+F_',1,'i') >0))  then  /* Se trata de una tabla de HECHOS  */
        ...
        if (v_tipo_particionado = 'D') then
          ...
          --DBMS_OUTPUT.put_line('PARTITION BY RANGE (CVE_DAY)');
          DBMS_OUTPUT.put_line('(');
          ...
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-126 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-119 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-119 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-112 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-112 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-105 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-105 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-98 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-98 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-91 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-91 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-84 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-84 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-77 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-77 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-70 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-70 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-63 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-63 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-56 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-56 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-49 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-49 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-42 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-42 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-35 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-35 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-28 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-28 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-21 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-21 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-14 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-14 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate-7 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate-7 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+7,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+7 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+14 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+14 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+21 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+21 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+28 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+28 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+35 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+35 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+42 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+42 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+49 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+49 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+56 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+56 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+63 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+63 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+70 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+70 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+77 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+77 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+84 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+84 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+91 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+91 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+98 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+98 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+105 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+105 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+112 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+112 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+119 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+119 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+126 ,'YYYYWW'))) || '),');   
          DBMS_OUTPUT.put_line('PARTITION ' || v_nombre_particion ||'_' || TO_CHAR(sysdate+126 ,'YYYYWW') || ' VALUES LESS THAN (' || TO_CHAR(TO_NUMBER(TO_CHAR(sysdate+134 ,'YYYYWW'))) || ')');   
          DBMS_OUTPUT.put_line(');');
