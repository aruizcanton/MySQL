...
BEGIN
  ...
  LOOP
    ...
    DBMS_OUTPUT.put_line('VALUES (' || reg.CVE_PROCESO || ', ' || reg.CVE_PASO || ', ''' || reg.CVE_RESULTADO || ''', '''  || reg.DESCRIPCION || ''', ''' || reg.ACCION || ''', ''' || reg.BAN_DIRECTIVA_EJECUTIVA || '''' || ', str_to_date(''' || to_char(reg.FCH_ALTA, 'DD/MM/YYYY') || ''', ''%d/%m/%Y''), ' || 'str_to_date(''' || to_char(reg.FCH_BAJA, 'DD/MM/YYYY') || ''', ''%d/%m/%Y''), str_to_date(''' || to_char(reg.FCH_REGISTRO, 'DD/MM/YYYY') || ''', ''%d/%m/%Y'')' || ');');
  END LOOP;
