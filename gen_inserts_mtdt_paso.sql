...
BEGIN
  ...
  LOOP
    ...
    DBMS_OUTPUT.put_line('VALUES (' || reg.CVE_PROCESO || ', ' || reg.CVE_PASO || ', ''' || reg.NOMBRE_PASO || ''', '''  || reg.TIPO_IMPLEMENTACION || ''', ''' || reg.TIPO_PASO || ''', ''' || reg.RESPONSABLE || ''', ''' || reg.AUTOR || ''', ''' || reg.VERSION || ''', str_to_date(''' || to_char(reg.FCH_ALTA, 'DD/MM/YYYY') || ''', ''%d/%m/%Y''), ''' || reg.ESTADO || ''', str_to_date(''' || to_char(reg.FCH_ESTADO, 'DD/MM/YYYY') || ''', ''%d/%m/%Y''), str_to_date(''' || to_char(reg.FCH_REGISTRO, 'DD/MM/YYYY') || ''', ''%d/%m/%Y'')' || ');');
  END LOOP;
