...
cursor MTDT_TABLA
  is
    ...
    (
    --'CMBD_USERS', 'CMBD_COURSES', 'CMBD_USERS_PROFILE', 'CMBD_USERS_RASGOS', 'CMBD_USERS_ROLES', 'CMBD_USERS_OPS'
    ...
    --'KRC_PERSONE_IDENTITY', 'KRC_ADDRESS', 'KRC_CONTACT', 'KRC_PRODUCT', 'KRC_PURCHASE', 'KRC_CATEGORY', 
    --'KRC_OFFER', 'KRC_USER', 'KRC_ORDER', 'KRC_PRODUCT_CATEGORY'
    --,'KRC_PRODUCT_TYPE', 'DMD_MESA', 'KRC_SUBURB', 'KRC_DISTRICT', 'KRC_CITY', 'KRC_PROVINCE', 'KRC_STATE', 
    --'KRC_COUNTRY', 'KRC_TAX_TYPE', 
    --'KRC_OFFER_COMP', 'KRC_PRODUCT_AVAIL', 'KRC_TEST',
    --'KRC_PERSONE_IDENTITY',
    --'KRC_ADDRESS', 'KRC_CONTACT', 'KRC_PRODUCT', 'KRC_USER', 'KRC_PRODUCT_CATEGORY', 'KRC_PRODUCT_AVAIL', 'KRC_TAX_TYPE','KRC_BANK',
    --'KRC_BANK_ACCOUNT', 'KRC_PAYROLL', 'KRC_PAYROLL_PERSONE', 'KRC_COUNTRY'
    
    'DWD_CUSTOMER', 'DWD_ACCOUNT', 'DWD_SUBSCRIBER', 'DWD_SALES', 'DWD_STATE_SUBSCRIBER'
    , 'DWD_STATE_ACCOUNT', 'DWD_STATE_CUSTOMER', 'DWD_STATE_SALE', 'DWD_OPERATOR', 'DWD_LOCATION'
    , 'DWD_PRODUCT_TYPE', 'DWD_SUBSCRIBER_TYPE', 'DWD_TICKET_TYPE', 'DWD_SUBSCRIBER_LANGUAGE'
    , 'DWD_NOTIFICATION_CHANNEL', 'DWD_ERROR_CODE', 'DWD_DESTINATION_NETWORK'
    , 'DWD_DESTINATION_ZONE', 'DWD_TRAFFIC_TYPE', 'DWD_ACCOUNT_CURRENCY', 'DWD_ACCOUNT_TYPE'
    , 'DWD_APN', 'DWD_SERVICE_TYPE', 'DWD_DISTRIBUTOR', 'DWD_POS', 'DWD_PAYMENT_METHOD'
    --'DWD_SUBSCRIBER'
    )
    --(
  ...
  is
    ...
    begin
      ...
      if lon_cadena > 0 then
        ...
        cadena_resul := regexp_replace(cadena_resul, '#VAR_FCH_INI_MES#', ' date_format(fch_datos_in, ''%Y%m01'') ');
  ...
  begin
      ...
      when 'LKUP' then
        ...
        if (reg_detalle_in.LKUP_COM_RULE is not null) then
          ...
          valor_retorno := 'CASE WHEN ' || trim(condicion) || ' THEN ifnull(' || v_alias || '.' || reg_detalle_in.VALUE || ', -2) ELSE ' || trim(constante) || ' END';
        else
            ...
            if (table_columns_lkup.COUNT > 1) then      /* Hay varios campos de condicion */
              ...
              if (INSTR(trim(l_registro2.TYPE), 'NUMBER') > 0) then
                ...
                /* (20160629) Angel Ruiz. NF: Se incluye la posibilidad de incluir el ALIAS en tablas de LKUP que sean SELECT */
                  ...
                  /* (20170929) Angel Ruiz. BUG. Aparece -2 cuando se trata de importes*/
                    ...
                    valor_retorno := valor_retorno || ') THEN -3 ELSE ' || 'ifnull(' || v_alias || '.' || procesa_campo_filter (reg_detalle_in.VALUE) || ', -2) END';
                  else
                    --valor_retorno := valor_retorno || ') THEN 0 ELSE ' || 'NVL(' || v_alias || '.' || procesa_campo_filter (reg_detalle_in.VALUE) || ', 0) END';
                    valor_retorno := valor_retorno || ') THEN 0 ELSE ' || 'ifnull(' || v_alias || '.' || procesa_campo_filter (reg_detalle_in.VALUE) || ', 0) END';
                  end if;
                end if;
                ...
                /* (20160629) Angel Ruiz. NF: Se incluye la posibilidad de incluir el ALIAS en tablas de LKUP que sean SELECT */
                  ...
                  valor_retorno := valor_retorno || ') THEN CAST(''1970-01-01'' AS DATE) ELSE ' || 'ifnull(' || procesa_campo_filter (reg_detalle_in.VALUE) || ', CAST(''2000-01-01'' AS DATE)) END';
                else
                  ...
                  valor_retorno := valor_retorno || ') THEN CAST(''1970-01-01'' AS DATE) ELSE ' || 'ifnull(' || v_alias || '.' || procesa_campo_filter (reg_detalle_in.VALUE) || ', CAST(''2000-01-01'' AS DATE)) END';
                end if;
              else
                if (v_alias_incluido = 1) then
                /* (20160629) Angel Ruiz. NF: Se incluye la posibilidad de incluir el ALIAS en tablas de LKUP que sean SELECT */
              ...
              if (instr(l_registro2.TYPE, 'NUMBER') > 0) then
                ...
                if (v_alias_incluido = 1) then
                  ...
                  valor_retorno :=  '    ifnull(' || procesa_campo_filter(reg_detalle_in.VALUE) || ', CAST(''2000-01-01'' AS DATE))';
                else
                  --valor_retorno :=  '    NVL(' || v_alias || '.' || reg_detalle_in.VALUE || ', CAST(''2000-01-01'' AS DATE))';
                  valor_retorno :=  '    ifnull(' || v_alias || '.' || reg_detalle_in.VALUE || ', CAST(''2000-01-01'' AS DATE))';
                end if;
              else
                if (v_alias_incluido = 1) then
                  --valor_retorno :=  '    NVL(' || procesa_campo_filter(reg_detalle_in.VALUE) || ', ''GENERICO'')';
                  valor_retorno :=  '    ifnull(' || procesa_campo_filter(reg_detalle_in.VALUE) || ', ''GENERICO'')';
                else
                  --valor_retorno :=  '    NVL(' || v_alias || '.' || reg_detalle_in.VALUE || ', ''GENERICO'')';
                  valor_retorno :=  '    ifnull(' || v_alias || '.' || reg_detalle_in.VALUE || ', ''GENERICO'')';
                end if;
              end if;
                ...
                if (UPPER(TRIM(l_registro2.TYPE)) = 'VARCHAR2') then    /* Estamos haciendo JOIN con la tabla de LookUp COD_* por un campo CARACTER */
                  if (TO_NUMBER(TRIM(l_registro2.LENGTH)) < 3 and l_registro2.NULABLE IS NULL) then
                    ...
                    if (regexp_instr(ie_column_lkup(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0 or regexp_instr(table_columns_lkup(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0) then
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) :=  'ifnull(' || transformo_decode(ie_column_lkup(indx), mi_tabla_base_name_alias, 0) || ', ''NI#'')' || ' = ' || transformo_decode(table_columns_lkup(indx), v_alias, 0);
                      ...
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) := 'ifnull(' || transformo_funcion(ie_column_lkup(indx), mi_tabla_base_name_alias) || ', ''NI#'')' ||  ' = ' || transformo_funcion(table_columns_lkup(indx), v_alias);
                      ...
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) :=  'ifnull(' || mi_tabla_base_name_alias || '.' || ie_column_lkup(indx) || ', ''NI#'')' || ' = ' || v_alias || '.' || table_columns_lkup(indx);
                    end if;
                  else
                    /* (20160302) Angel Ruiz. NF: DECODE en las columnas de LookUp */
                    if (regexp_instr(ie_column_lkup(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0 or regexp_instr(table_columns_lkup(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0) then
              ...
              if (v_existe_valor = true) then /* (20170110) Angel Ruiz */
                if (UPPER(TRIM(l_registro2.TYPE))='VARCHAR2') then    /* Estamos haciendo JOIN con la tabla de LookUp COD_* por un campo CARACTER */
                  if (TO_NUMBER(TRIM(l_registro2.LENGTH)) < 3 and l_registro2.NULABLE IS NULL) then
                    /* (20160302) Angel Ruiz. NF: DECODE en las columnas de LookUp */
                    if (regexp_instr(ie_column_lkup(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0 or regexp_instr(table_columns_lkup(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0) then
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) :=  ' AND ifnull(' || transformo_decode(ie_column_lkup(indx), mi_tabla_base_name_alias, 0) || ', ''NI#'')' || ' = ' || transformo_decode(table_columns_lkup(indx), v_alias, 0);
                    elsif (regexp_instr(ie_column_lkup(indx), '[Ii][Ff][Nn][Uu][Ll][Ll]') > 0 or regexp_instr(table_columns_lkup(indx), '[Ii][Ff][Nn][Uu][Ll][Ll]') > 0) then
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) :=  ' AND ifnull(' || transformo_funcion(ie_column_lkup(indx), mi_tabla_base_name_alias) || ', ''NI#'')' || ' = ' || transformo_funcion(table_columns_lkup(indx), v_alias);
                    elsif (regexp_instr(ie_column_lkup(indx), '[Uu][Pp][Pp][Ee][Rr]') > 0 or regexp_instr(table_columns_lkup(indx), '[Uu][Pp][Pp][Ee][Rr]') > 0) then
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) :=  ' AND ifnull(' || transformo_funcion(ie_column_lkup(indx), mi_tabla_base_name_alias) || ', ''NI#'')' || ' = ' || transformo_funcion(table_columns_lkup(indx), v_alias);
                    elsif (regexp_instr(ie_column_lkup(indx), '[Rr][Ee][Pp][Ll][Aa][Cc][Ee]') > 0 or regexp_instr(table_columns_lkup(indx), '[Rr][Ee][Pp][Ll][Aa][Cc][Ee]') > 0) then
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) :=  ' AND ifnull(' || transformo_funcion(ie_column_lkup(indx), mi_tabla_base_name_alias) || ', ''NI#'')' || ' = ' || transformo_funcion(table_columns_lkup(indx), v_alias);
                      ...
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) :=  ' AND ifnull(' || mi_tabla_base_name_alias || '.' || ie_column_lkup(indx) || ', ''NI#'')' || ' = ' || v_alias || '.' || table_columns_lkup(indx);
                    end if;
                  else
                    /* (20160302) Angel Ruiz. NF: DECODE en las columnas de LookUp */
                    if (regexp_instr(ie_column_lkup(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0 or regexp_instr(table_columns_lkup(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0) then
            ...
            if (l_WHERE_ON_clause.count = 1) then /* si es el primer campo del WHERE */
                ...
                if (UPPER(TRIM(l_registro2.TYPE)) = 'VARCHAR2') then    /* Estamos haciendo JOIN con la tabla de LookUp COD_* por un campo CARACTER */
                  if (TO_NUMBER(TRIM(l_registro2.LENGTH)) < 3 and l_registro2.NULABLE IS NULL) then
                    if (regexp_instr(reg_detalle_in.IE_COLUMN_LKUP, '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0 or regexp_instr(reg_detalle_in.TABLE_COLUMN_LKUP, '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0) then
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) := 'ifnull(' || transformo_decode(reg_detalle_in.IE_COLUMN_LKUP, mi_tabla_base_name_alias, 0) || ', ''NI#'')' ||  ' = ' || transformo_decode(reg_detalle_in.TABLE_COLUMN_LKUP, v_alias, 0);
                      ...
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) := 'ifnull(' || transformo_funcion(reg_detalle_in.IE_COLUMN_LKUP, mi_tabla_base_name_alias) || ', ''NI#'')' ||  ' = ' || transformo_funcion(reg_detalle_in.TABLE_COLUMN_LKUP, v_alias);
                      ...
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) := 'ifnull(' || mi_tabla_base_name_alias || '.' || reg_detalle_in.IE_COLUMN_LKUP || ', ''NI#'')' ||  ' = ' || v_alias || '.' || reg_detalle_in.TABLE_COLUMN_LKUP;
                    end if;
                  else
                    if (regexp_instr(reg_detalle_in.IE_COLUMN_LKUP, '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0 or regexp_instr(reg_detalle_in.TABLE_COLUMN_LKUP, '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0) then
              ...
              if (v_existe_valor = true) then
                if (UPPER(TRIM(l_registro2.TYPE)) = 'VARCHAR2') then     /* Estamos haciendo JOIN con la tabla de LookUp COD_* por un campo CARACTER */
                  if (TO_NUMBER(TRIM(l_registro2.LENGTH)) < 3 and l_registro2.NULABLE IS NULL) then
                    if (regexp_instr(reg_detalle_in.IE_COLUMN_LKUP, '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0 or regexp_instr(reg_detalle_in.TABLE_COLUMN_LKUP, '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0) then
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) :=  ' AND ifnull(' || transformo_decode(reg_detalle_in.IE_COLUMN_LKUP, mi_tabla_base_name_alias, 0) || ', ''NI#'')' || ' = ' || transformo_decode(reg_detalle_in.TABLE_COLUMN_LKUP, v_alias, 0);
                    elsif (regexp_instr(reg_detalle_in.IE_COLUMN_LKUP, '[Ii][Ff][Nn][Uu][Ll][Ll]') > 0 or regexp_instr(reg_detalle_in.TABLE_COLUMN_LKUP, '[Ii][Ff][Nn][Uu][Ll][Ll]') > 0) then
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) :=  ' AND ifnull(' || transformo_funcion(reg_detalle_in.IE_COLUMN_LKUP, mi_tabla_base_name_alias) || ', ''NI#'')' || ' = ' || transformo_funcion(reg_detalle_in.TABLE_COLUMN_LKUP, v_alias);
                    elsif (regexp_instr(reg_detalle_in.IE_COLUMN_LKUP, '[Uu][Pp][Pp][Ee][Rr]') > 0 or regexp_instr(reg_detalle_in.TABLE_COLUMN_LKUP, '[Uu][Pp][Pp][Ee][Rr]') > 0) then
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) :=  ' AND ifnull(' || transformo_funcion(reg_detalle_in.IE_COLUMN_LKUP, mi_tabla_base_name_alias) || ', ''NI#'')' || ' = ' || transformo_funcion(reg_detalle_in.TABLE_COLUMN_LKUP, v_alias);
                    elsif (regexp_instr(reg_detalle_in.IE_COLUMN_LKUP, '[Rr][Ee][Pp][Ll][Aa][Cc][Ee]') > 0 or regexp_instr(reg_detalle_in.TABLE_COLUMN_LKUP, '[Rr][Ee][Pp][Ll][Aa][Cc][Ee]') > 0) then
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) :=  ' AND ifnull(' || transformo_funcion(reg_detalle_in.IE_COLUMN_LKUP, mi_tabla_base_name_alias) || ', ''NI#'')' || ' = ' || transformo_funcion(reg_detalle_in.TABLE_COLUMN_LKUP, v_alias);
                      ...
                      l_WHERE_ON_clause(l_WHERE_ON_clause.last) :=  ' AND ifnull(' || mi_tabla_base_name_alias || '.' || reg_detalle_in.IE_COLUMN_LKUP || ', ''NI#'')' || ' = ' || v_alias || '.' || reg_detalle_in.TABLE_COLUMN_LKUP;
                    end if;
                  else
                    if (regexp_instr(reg_detalle_in.IE_COLUMN_LKUP, '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0 or regexp_instr(reg_detalle_in.TABLE_COLUMN_LKUP, '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0) then
      ...
      when 'SEQ' then
        ...
        --end if;
        --valor_retorno := '    ' || ESQUEMA_DM || '.nextval(''seq_' ||  nombre_tabla_reducido || ''')';
        ...
        --l_FROM (l_FROM.LAST) := ' LEFT OUTER JOIN (SELECT ULT_VAL maximo from ' || OWNER_MTDT || '.MTDT_SEQUENCIAS WHERE ID_SEQ=''SEQ_' || substr(regexp_substr(reg_detalle_in.TABLE_COLUMN, '_[A-Za-z_]+$'), 2) || ''') ' || 'SEQ_' || substr(regexp_substr(reg_detalle_in.TABLE_COLUMN, '_[A-Za-z_]+$'), 2);
        --valor_retorno := '    ' || reg_detalle_in.value || '.' || 'maximo + (row_number() over (order by ' || reg_detalle_in.table_name || '.' || reg_detalle_in.TABLE_COLUMN || '))' ;
        /* (20170630) Angel Ruiz. BUG*/
        ...
        /* (20170703) Angel Ruiz. BUG. Puede tener TABLE_BASE_NAME un ALIAS lo que genera mal el valor_retorno */
        ...
        --else
          ...
          --end if;
        --end if;
        --valor_retorno := '    ' || '(SEQ_' || substr(regexp_substr(reg_detalle_in.TABLE_COLUMN, '_[A-Za-z_]+$'), 2) || '.' || 'maximo + (row_number() over (order by ' || reg_detalle_in.TABLE_BASE_NAME || '.' || reg_detalle_in.VALUE || ')))';
        --valor_retorno := '    ' || '(SEQ_' || substr(regexp_substr(reg_detalle_in.TABLE_COLUMN, '_[A-Za-z_]+$'), 2) || '.' || 'maximo + (row_number() over (order by ' || v_alias_table_base_name || '.' || reg_detalle_in.VALUE || ')))';
        /* (20170703) Angel Ruiz. FIN BUG*/
...
--          valor_retorno := '     ' || 'TO_DATE (fch_carga_in, ''YYYYMMDD'')';
          --valor_retorno := '     ' || 'fch_datos_in';        
          --valor_retorno := '     ' || '''#VAR_FCH_DATOS#''';  /* (20161208) Angel Ruiz */      
          valor_retorno := '     ' || 'date_format(fch_carga_in, ''%Y%m%d'')';  /* (20210805) Angel Ruiz */      
        end if;
...
/*******************************************/
/*******************************************/
      UTL_FILE.put_line(fich_salida_pkg, 'DROP PROCEDURE IF EXISTS ' || ESQUEMA_DM || '.neh_' || nombre_proceso || ';');
      UTL_FILE.put_line(fich_salida_pkg, 'DELIMITER //');
      UTL_FILE.put_line(fich_salida_pkg, 'CREATE PROCEDURE ' || ESQUEMA_DM || '.neh_' || nombre_proceso || ' (IN fch_carga_in VARCHAR(8), IN fch_datos_in VARCHAR(8), IN forzado_in VARCHAR(1))');
      UTL_FILE.put_line(fich_salida_pkg, 'BEGIN');
      ...
      UTL_FILE.put_line(fich_salida_pkg, '    CALL ' || ESQUEMA_DM || '.inserta_monitoreo (''' || nombre_fich_carga || ''', 1, 1, inicio_paso_tmr, CURRENT_TIMESTAMP(), STR_TO_DATE(fch_datos_in,''%Y%m%d''), STR_TO_DATE(fch_carga_in,''%Y%m%d''), 0, 0, 0, 0, 0);');
      UTL_FILE.put_line(fich_salida_pkg, '    RESIGNAL;');
      UTL_FILE.put_line(fich_salida_pkg, '  END;');
      UTL_FILE.put_line(fich_salida_pkg, '  SET numero_reg_new = 0;');
      ...
      UTL_FILE.put_line(fich_salida_pkg, '  select concat(''Inicio del proceso de carga: '', ''' || ESQUEMA_DM || '.neh_' || nombre_proceso || ''', ''.'');');
      ...
      UTL_FILE.put_line(fich_salida_pkg, '  CALL ' || ESQUEMA_DM || '.siguiente_paso(''' || nombre_fich_carga || ''', STR_TO_DATE(fch_datos_in, ''%Y%m%d''), STR_TO_DATE(fch_carga_in, ''%Y%m%d''), siguiente_paso_a_ejecutar);');
      ...
      LOOP
        ...
        then
          ...
          --UTL_FILE.put_line(fich_salida_pkg, '    SELECT ' || OWNER_DM || '.nreg_' || nombre_proceso || ' (fch_carga_in, fch_datos_in) into numero_reg_new;');
          --UTL_FILE.put_line(fich_salida_pkg, '    select concat(''El numero de registros insertados es:'', numero_reg_new);');
          ...
          UTL_FILE.put_line(fich_salida_pkg, '    select concat(''El numero de registros insertados en el escenario ' || lista_scenarios_presentes(indx) || ' es:'', numero_reg_' || lista_scenarios_presentes(indx) || ');');
          UTL_FILE.put_line(fich_salida_pkg, '    SET numero_reg_updt = numero_reg_updt + numero_reg_' || lista_scenarios_presentes(indx) || ';');
      ...
      LOOP
        ...
        then
          ...
          UTL_FILE.put_line(fich_salida_pkg, '    SELECT numero_reg_' || lista_scenarios_presentes(indx) || ' + ' || ESQUEMA_DM || '.' || lista_scenarios_presentes(indx) || '_reg_' || nombre_proceso || ' (fch_carga_in, fch_datos_in) into numero_reg_' || lista_scenarios_presentes(indx) || ';');
          UTL_FILE.put_line(fich_salida_pkg, '    select concat(''El numero de registros insertados en el escenario ' || lista_scenarios_presentes(indx) || ' es:'', numero_reg_' || lista_scenarios_presentes(indx) || ');');
          UTL_FILE.put_line(fich_salida_pkg, '    SET numero_reg_hist = numero_reg_hist + numero_reg_' || lista_scenarios_presentes(indx) || ';');
      ...
      UTL_FILE.put_line(fich_salida_pkg, '    CALL ' || ESQUEMA_DM || '.inserta_monitoreo (''' || nombre_fich_carga || ''', 1, 0, inicio_paso_tmr, CURRENT_TIMESTAMP(), STR_TO_DATE(fch_datos_in,''%Y%m%d''), STR_TO_DATE(fch_carga_in,''%Y%m%d''), numero_reg_new, numero_reg_updt, numero_reg_hist, numero_reg_read, 0);');
      UTL_FILE.put_line(fich_salida_pkg, '    commit;');
      UTL_FILE.put_line(fich_salida_pkg, '  end if;');
      UTL_FILE.put_line(fich_salida_pkg, 'END');
      UTL_FILE.put_line(fich_salida_pkg, '//');
      UTL_FILE.put_line(fich_salida_pkg, 'DELIMITER ;');
      UTL_FILE.put_line(fich_salida_pkg, 'DROP PROCEDURE IF EXISTS ' || ESQUEMA_DM || '.lex_' || nombre_proceso || ';');
      UTL_FILE.put_line(fich_salida_pkg, 'DELIMITER //');
      UTL_FILE.put_line(fich_salida_pkg, 'CREATE PROCEDURE ' || ESQUEMA_DM || '.lex_' || nombre_proceso || ' (IN fch_carga_in VARCHAR(8), IN fch_datos_in VARCHAR(8), IN forzado_in VARCHAR(1))');
      UTL_FILE.put_line(fich_salida_pkg, 'BEGIN');
      UTL_FILE.put_line(fich_salida_pkg, '  DECLARE num_reg INT;');
      UTL_FILE.put_line(fich_salida_pkg, '  DECLARE inicio_paso_tmr TIMESTAMP;');
      UTL_FILE.put_line(fich_salida_pkg, '  DECLARE siguiente_paso_a_ejecutar int;');
      UTL_FILE.put_line(fich_salida_pkg, '  DECLARE code CHAR(5) DEFAULT ''00000'';');
      UTL_FILE.put_line(fich_salida_pkg, '  DECLARE msg TEXT;');
      ...
      UTL_FILE.put_line(fich_salida_pkg, '    select concat(''Error code: '', @errno, ''('', @code, ''). '', ''Mensaje: '', @msg);');
      UTL_FILE.put_line(fich_salida_pkg, '    CALL ' || ESQUEMA_DM || '.inserta_monitoreo (''' || nombre_fich_exchange || ''', siguiente_paso_a_ejecutar, 1, inicio_paso_tmr, CURRENT_TIMESTAMP(), STR_TO_DATE(fch_datos_in,''%Y%m%d''), STR_TO_DATE(fch_carga_in,''%Y%m%d''), num_reg, 0, 0, 0, 0);');
      UTL_FILE.put_line(fich_salida_pkg, '    RESIGNAL;');
      UTL_FILE.put_line(fich_salida_pkg, '  END;');
      UTL_FILE.put_line(fich_salida_pkg, '  SET siguiente_paso_a_ejecutar=0;');
      UTL_FILE.put_line(fich_salida_pkg, '  SET num_reg=0;');
      UTL_FILE.put_line(fich_salida_pkg, '');
      UTL_FILE.put_line(fich_salida_pkg, '  select concat(''Inicio del proceso de carga: '', ''' || ESQUEMA_DM || '.lex_' || nombre_proceso || ''', ''.'');');
      /* (20191011) Angel Ruiz. BUG.*/
      --UTL_FILE.put_line(fich_salida_pkg, '  CALL COMBI.siguiente_paso(''' || '.lex_' || nombre_proceso || ''', STR_TO_DATE(fch_datos_in, ''%Y%m%d''), STR_TO_DATE(fch_carga_in, ''%Y%m%d''), siguiente_paso_a_ejecutar);');
      UTL_FILE.put_line(fich_salida_pkg, '  CALL ' || ESQUEMA_DM || '.siguiente_paso(''' || '.lex_' || nombre_proceso || ''', STR_TO_DATE(fch_datos_in, ''%Y%m%d''), STR_TO_DATE(fch_carga_in, ''%Y%m%d''), siguiente_paso_a_ejecutar);');
      /* (20191011) Angel Ruiz. FIN*/
      UTL_FILE.put_line(fich_salida_pkg, '  if (forzado_in = ''F'') then');
      UTL_FILE.put_line(fich_salida_pkg, '    set siguiente_paso_a_ejecutar := 1;');
      UTL_FILE.put_line(fich_salida_pkg, '  end if;');
      UTL_FILE.put_line(fich_salida_pkg, '  if (siguiente_paso_a_ejecutar = 1) then');
      UTL_FILE.put_line(fich_salida_pkg, '    /* Este tipo de procesos "ex"  tienen dos pasos */');
      UTL_FILE.put_line(fich_salida_pkg, '    /* Comienza en el primer paso */');
      UTL_FILE.put_line(fich_salida_pkg, '    set inicio_paso_tmr = CURRENT_TIMESTAMP();');
      UTL_FILE.put_line(fich_salida_pkg, '    SELECT COUNT(*) INTO num_reg FROM ' || ESQUEMA_DM || '.T_' || nombre_tabla_reducido || ';');
      
      UTL_FILE.put_line(fich_salida_pkg, '    /* Truncamos la tabla antes de insertar los nuevos registros por si se lanza dos veces*/');
      UTL_FILE.put_line(fich_salida_pkg, '    TRUNCATE TABLE ' || ESQUEMA_DM || '.' || reg_tabla.TABLE_NAME || ';');
      UTL_FILE.put_line(fich_salida_pkg, '    CALL ' || ESQUEMA_DM || '.inserta_monitoreo (''' || nombre_fich_exchange || ''', 1, 0, inicio_paso_tmr, CURRENT_TIMESTAMP(), STR_TO_DATE(fch_datos_in,''%Y%m%d''), STR_TO_DATE(fch_carga_in,''%Y%m%d''), 0, 0, 0, 0, num_reg);');
      UTL_FILE.put_line(fich_salida_pkg, '    commit;');
      UTL_FILE.put_line(fich_salida_pkg, '    /* comienza el segundo paso */');
      UTL_FILE.put_line(fich_salida_pkg, '    set inicio_paso_tmr = CURRENT_TIMESTAMP();');
      UTL_FILE.put_line(fich_salida_pkg, '    set siguiente_paso_a_ejecutar = siguiente_paso_a_ejecutar+1;');
      UTL_FILE.put_line(fich_salida_pkg, '    INSERT');
      ...
      UTL_FILE.put_line(fich_salida_pkg, '    ;');
      UTL_FILE.put_line(fich_salida_pkg, '    CALL ' || ESQUEMA_DM || '.inserta_monitoreo (''' || nombre_fich_exchange || ''', 2, 0, inicio_paso_tmr, CURRENT_TIMESTAMP(), STR_TO_DATE(fch_datos_in,''%Y%m%d''), STR_TO_DATE(fch_carga_in,''%Y%m%d''), num_reg, 0, 0, 0, 0);');
      UTL_FILE.put_line(fich_salida_pkg, '    commit;');
      ...
      UTL_FILE.put_line(fich_salida_pkg, '    FROM ' || ESQUEMA_DM || '.T_' || nombre_tabla_reducido);
      UTL_FILE.put_line(fich_salida_pkg, '    ;');
      UTL_FILE.put_line(fich_salida_pkg, '    CALL ' || ESQUEMA_DM || '.inserta_monitoreo (''' || nombre_fich_exchange || ''', 2, 0, inicio_paso_tmr, CURRENT_TIMESTAMP(), STR_TO_DATE(fch_datos_in,''%Y%m%d''), STR_TO_DATE(fch_carga_in,''%Y%m%d''), 0, 0, 0, 0, num_reg);');
      UTL_FILE.put_line(fich_salida_pkg, '    commit;');
  ...
  /***********************/
      ...
      UTL_FILE.put_line(fich_salida_load, '#                                                                           #');
      UTL_FILE.put_line(fich_salida_load, '# Historia : 08-Junio-2018 -> Creacion                                    #');
      ...
      UTL_FILE.put_line(fich_salida_load, '   EjecutaInserMonitoreo ${BD_SID} ${BD_USUARIO} ${' || NAME_DM || '_SQL}/insert_monitoreo.sql ' || 'load_ne_' || nombre_proceso || '.sh 1 1 "''${INICIO_PASO_TMR}''" current_timestamp ${FCH_DATOS} ${FCH_CARGA}' || ' >> ${' || NAME_DM || '_TRAZAS}/load_ne_' || nombre_proceso || '_${FECHA_HORA}' || '.log 2>&' || '1' );
      ...
      --UTL_FILE.put_line(fich_salida_sh, '   BD_SID=$1');
      --UTL_FILE.put_line(fich_salida_sh, '   USER=$2');
      UTL_FILE.put_line(fich_salida_load, '   EjecutaInserMonitoreo ${BD_SID} ${BD_USUARIO} ${' || NAME_DM || '_SQL}/insert_monitoreo.sql ' || 'load_ne_' || nombre_proceso || '.sh 1 0 "''${INICIO_PASO_TMR}''" current_timestamp ${FCH_DATOS} ${FCH_CARGA} ${TOT_INSERTADOS} 0 0 ${TOT_LEIDOS} ${TOT_RECHAZADOS}' || ' >> ${' || NAME_DM || '_TRAZAS}/load_ne_' || nombre_proceso || '_${FECHA_HORA}' || '.log 2>&' || '1' );
      UTL_FILE.put_line(fich_salida_load, '   if [ $? -ne 0 ]');
      UTL_FILE.put_line(fich_salida_load, '   then');
      ...
      UTL_FILE.put_line(fich_salida_load, 'INTERFAZ=' || 'load_ne_' || nombre_proceso);
      UTL_FILE.put_line(fich_salida_load, '');
      UTL_FILE.put_line(fich_salida_load, '################################################################################');
      ...
      UTL_FILE.put_line(fich_salida_load, '################################################################################');
      UTL_FILE.put_line(fich_salida_load, 'if [ "`/sbin/ifconfig -a | grep ''192.168.2.'' | awk ''{print $2}''`" = "192.168.2.109" ]||[ "`/sbin/ifconfig -a | grep ''192.168.2.'' | awk ''{print $2}''`" = "192.168.2.109" ]; then');
      ...
      --UTL_FILE.put_line(fich_salida_load, '  BD_MVNO=UBITEL');
      --UTL_FILE.put_line(fich_salida_load, '  USR_MVNO=ubitel_own');
      --UTL_FILE.put_line(fich_salida_load, '  PWD_MVNO=');
      ...
      UTL_FILE.put_line(fich_salida_load, 'mysql -Ns -u ${BD_USUARIO} -p${BD_CLAVE} -D ${BD_SID} -h ${HOST_' || NAME_DM || '} << EOF >> ${' || NAME_DM || '_TRAZAS}/load_ne_' || nombre_proceso || '_${FECHA_HORA}' || '.log ' ||  '2>&' || '1');
      UTL_FILE.put_line(fich_salida_load, '');
      UTL_FILE.put_line(fich_salida_load, '  ' || 'call ' || ESQUEMA_DM || '.' || 'neh_' || nombre_proceso || ' (''${FCH_CARGA}'', ''${FCH_DATOS}'', ''${BAN_FORZADO}'');');
      ...
      UTL_FILE.put_line(fich_salida_load, '  SUBJECT="${INTERFAZ}: Surgio un error en el sqlplus en la llamada a pre_' || nombre_proceso || '. Error:  ${err_salida}."');
      UTL_FILE.put_line(fich_salida_load, '  ${SHELL_SMS} "${TELEFONOS_DWH}" "${SUBJECT}"');
      ...
      UTL_FILE.put_line(fich_salida_exchange, '#                                                                           #');
      UTL_FILE.put_line(fich_salida_exchange, '# Archivo    :       load_ex_' ||  nombre_proceso || '.sh                            #');
      UTL_FILE.put_line(fich_salida_exchange, '#                                                                           #');
      ...
      UTL_FILE.put_line(fich_salida_exchange, '#                                                                           #');
      UTL_FILE.put_line(fich_salida_exchange, '# Historia : 08-Junio-2018 -> Creacion                                    #');
      ...
      --UTL_FILE.put_line(fich_salida_sh, '   BD_SID=$1');
      --UTL_FILE.put_line(fich_salida_sh, '   USER=$2');
      UTL_FILE.put_line(fich_salida_exchange, '   EjecutaInserMonitoreo ${BD_SID} ${BD_USUARIO} ${' || NAME_DM || '_SQL}/insert_monitoreo.sql ' || 'load_ex_' || nombre_proceso || '.sh 1 1 "''${INICIO_PASO_TMR}''" current_timestamp ${FCH_DATOS} ${FCH_CARGA}' || ' >> ${' || NAME_DM || '_TRAZAS}/load_ex_' || nombre_proceso || '_${FECHA_HORA}' || '.log 2>&' || '1' );
      ...
      --UTL_FILE.put_line(fich_salida_sh, '   BD_SID=$1');
      --UTL_FILE.put_line(fich_salida_sh, '   USER=$2');
      UTL_FILE.put_line(fich_salida_exchange, '   EjecutaInserMonitoreo ${BD_SID} ${BD_USUARIO} ${' || NAME_DM || '_SQL}/insert_monitoreo.sql ' || 'load_ex_' || nombre_proceso || '.sh 1 0 "''${INICIO_PASO_TMR}''" current_timestamp ${FCH_DATOS} ${FCH_CARGA} ${TOT_INSERTADOS} 0 0 ${TOT_LEIDOS} ${TOT_RECHAZADOS}' || ' >> ${' || NAME_DM || '_TRAZAS}/load_ex_' || nombre_proceso || '_${FECHA_HORA}' || '.log 2>&' || '1' );
      UTL_FILE.put_line(fich_salida_exchange, '   if [ $? -ne 0 ]');
      UTL_FILE.put_line(fich_salida_exchange, '   then');
      ...
      UTL_FILE.put_line(fich_salida_exchange, 'echo "${0}" > ${' || NAME_DM || '_TRAZAS}/load_ex_' || nombre_proceso || '_${FECHA_HORA}' || '.log ');
      UTL_FILE.put_line(fich_salida_exchange, 'echo "Inicia Proceso: `date +%d/%m/%Y\ %H:%M:%S`"  >> ${' || NAME_DM || '_TRAZAS}/load_ex_' || nombre_proceso || '_${FECHA_HORA}' || '.log ');
      UTL_FILE.put_line(fich_salida_exchange, 'echo "Fecha de Carga: ${FCH_CARGA}"  >> ${' || NAME_DM || '_TRAZAS}/load_ex_' || nombre_proceso || '_${FECHA_HORA}' || '.log ');
      UTL_FILE.put_line(fich_salida_exchange, 'echo "Fecha de Datos: ${FCH_DATOS}"  >> ${' || NAME_DM || '_TRAZAS}/load_ex_' || nombre_proceso || '_${FECHA_HORA}' || '.log ');
      UTL_FILE.put_line(fich_salida_exchange, 'echo "Forzado: ${BAN_FORZADO}"  >> ${' || NAME_DM || '_TRAZAS}/load_ex_' || nombre_proceso || '_${FECHA_HORA}' || '.log ');
      UTL_FILE.put_line(fich_salida_exchange, '################################################################################');
      ...
      --UTL_FILE.put_line(fich_salida_sh, 'REQ_NUM="Req89208"');
      --UTL_FILE.put_line(fich_salida_sh, 'INTERFAZ=' || v_REQ_NUMER || '_load_SA_' || reg_summary.CONCEPT_NAME);
      UTL_FILE.put_line(fich_salida_exchange, 'INTERFAZ=' || 'load_ex_' || nombre_proceso);
      UTL_FILE.put_line(fich_salida_exchange, '');
      UTL_FILE.put_line(fich_salida_exchange, '################################################################################');
      ...
      UTL_FILE.put_line(fich_salida_exchange, '################################################################################');
      UTL_FILE.put_line(fich_salida_exchange, 'if [ "`/sbin/ifconfig -a | grep ''192.168.2.'' | awk ''{print $2}''`" = "192.168.2.109" ]||[ "`/sbin/ifconfig -a | grep ''192.168.2.'' | awk ''{print $2}''`" = "192.168.2.109" ]; then');
      ...
      UTL_FILE.put_line(fich_salida_exchange, '  TELEFONOS_DWH=`cat ${' || NAME_DM || '_CONFIGURACION}/TelefonosMantto.txt`');
      UTL_FILE.put_line(fich_salida_exchange, '  TELEFONOS_USUARIOS=`cat ${' || NAME_DM || '_CONFIGURACION}/TELEFONOS_USUARIOS.txt`');
      --UTL_FILE.put_line(fich_salida_exchange, '  BD_MVNO=BIDESA');
      --UTL_FILE.put_line(fich_salida_exchange, '  USR_MVNO=ubitel_own');
      --UTL_FILE.put_line(fich_salida_exchange, '  PWD_MVNO=');
      UTL_FILE.put_line(fich_salida_exchange, 'fi');
      UTL_FILE.put_line(fich_salida_exchange, '');
      ...
      UTL_FILE.put_line(fich_salida_exchange, '# Llamada a mySql');
      UTL_FILE.put_line(fich_salida_exchange, 'mysql -Ns -u ${BD_USUARIO} -p${BD_CLAVE} -D ${BD_SID} -h ${HOST_' || NAME_DM || '} << EOF >> ${' || NAME_DM || '_TRAZAS}/load_ex_' || nombre_proceso || '_${FECHA_HORA}' || '.log ' ||  '2>&' || '1');
      UTL_FILE.put_line(fich_salida_exchange, '');
      UTL_FILE.put_line(fich_salida_exchange, '  ' || 'call ' || ESQUEMA_DM || '.' || 'lex_' || nombre_proceso || ' (''${FCH_CARGA}'', ''${FCH_DATOS}'', ''${BAN_FORZADO}'');');
      ...
      UTL_FILE.put_line(fich_salida_exchange, '  SUBJECT="${INTERFAZ}: Surgio un error en el sqlplus en la llamada a pre_' || nombre_proceso || '. Error:  ${err_salida}."');
      UTL_FILE.put_line(fich_salida_exchange, '  ${SHELL_SMS} "${TELEFONOS_DWH}" "${SUBJECT}"');
      UTL_FILE.put_line(fich_salida_exchange, '  echo ${SUBJECT} >> ' || '${' || NAME_DM || '_TRAZAS}/' || 'load_ex' || '_' || nombre_proceso || '_${FECHA_HORA}.log');        
      UTL_FILE.put_line(fich_salida_exchange, '  echo `date` >> ' || '${' || NAME_DM || '_TRAZAS}/' || 'load_ex' || '_' || nombre_proceso || '_${FECHA_HORA}.log');
      --UTL_FILE.put_line(fich_salida_exchange, '  InsertaFinFallido');
      UTL_FILE.put_line(fich_salida_exchange, '  exit 1');
      UTL_FILE.put_line(fich_salida_exchange, 'fi');
      UTL_FILE.put_line(fich_salida_exchange, 'echo "El proceso ' || nombre_fich_exchange || ' se ha realizado correctamente." >> ' || '${' || NAME_DM || '_TRAZAS}/' || 'load_ex_' || reg_tabla.TABLE_NAME || '_${FECHA_HORA}.log');
    ...
    if (upper(reg_tabla.TABLE_TYPE) = 'I') 
    then
      dbms_output.put_line ('Estoy en el primer LOOP de las tablas tipo I. La tabla que tengo es: ' || reg_tabla.TABLE_NAME);
      --nombre_tabla_reducido := substr(reg_tabla.TABLE_NAME, 4); /* Le quito al nombre de la tabla los caracteres SA_  */
      nombre_tabla_reducido := substr(reg_tabla.TABLE_NAME, instr(reg_tabla.TABLE_NAME, '_')+1); /* Le quito al nombre de la tabla los caracteres SA_  */
      ...
      nombre_fich_carga := 'load_SA_' || nombre_tabla_reducido || '.sh';
      --nombre_fich_pkg := 'pkg_' || reg_tabla.TABLE_NAME || '.sql';
      nombre_fich_pkg := 'pkg_' || reg_tabla.TABLE_NAME || '.sql'; /* Angel Ruiz (20141201) Hecho porque hay paquetes que no compilan */
      --nombre_fich_exchange := 'load_ex_' || reg_tabla.TABLE_NAME || '.sh';
      --nombre_fich_hist := 'load_dh_' || reg_tabla.TABLE_NAME || '.sh';
      ...
      loop
        ...
        exit when MTDT_SCENARIO%NOTFOUND;
        dbms_output.put_line ('Estoy en el segundo LOOP. La tabla que tengo es: ' || reg_tabla.TABLE_NAME || '. El escenario es: ' || reg_scenario.SCENARIO);
        /* (20170421) Angel Ruiz. En este tipo de tablas solo tendremos un scenario ya que son tablas de tipo integracion */
        UTL_FILE.put_line(fich_salida_pkg, '');
        UTL_FILE.put_line(fich_salida_pkg, '-- ### ESCENARIO : ' || reg_scenario.SCENARIO || ' ###');
        UTL_FILE.put_line(fich_salida_pkg, '');
        UTL_FILE.put_line(fich_salida_pkg, 'TRUNCATE TABLE ' || ESQUEMA_DM || '.' || reg_scenario.TABLE_NAME || ';');
        UTL_FILE.put_line(fich_salida_pkg, '');
        UTL_FILE.put_line(fich_salida_pkg, 'INSERT');
        UTL_FILE.put_line(fich_salida_pkg, 'INTO TABLE ' || ESQUEMA_DM || '.' || reg_scenario.TABLE_NAME);
        ...
        UTL_FILE.put_line(fich_salida_load, '# Telefonica Moviles Mexico SA DE CV                                        #');
        UTL_FILE.put_line(fich_salida_load, '#                                                                           #');
        UTL_FILE.put_line(fich_salida_load, '# Archivo    :       load_SA_' ||  nombre_tabla_reducido || '.sh                            #');
        UTL_FILE.put_line(fich_salida_load, '#                                                                           #');
        UTL_FILE.put_line(fich_salida_load, '# Autor      : <SYNAPSYS>.                                                  #');
        UTL_FILE.put_line(fich_salida_load, '# Proposito  : Shell que ejecuta los procesos de STAGING para ' || NAME_DM || '.        #');
        UTL_FILE.put_line(fich_salida_load, '# Parametros :                                                              #');
        ...
        UTL_FILE.put_line(fich_salida_load, 'MTDT_PROCESO.NOMBRE_PROCESO = ''load_SA_' || nombre_tabla_reducido || '.sh'';');
        UTL_FILE.put_line(fich_salida_load, '!quit');
        UTL_FILE.put_line(fich_salida_load, 'EOF');
        UTL_FILE.put_line(fich_salida_load, '  if [ $? -ne 0 ]');
        UTL_FILE.put_line(fich_salida_load, '  then');
        ...
        UTL_FILE.put_line(fich_salida_load, 'fi');
        UTL_FILE.put_line(fich_salida_load, NAME_DM || '_TRAZAS=${' || NAME_DM || '_TRAZAS}/${FCH_CARGA}');
        UTL_FILE.put_line(fich_salida_load, 'echo "${0}" > ${' || NAME_DM || '_TRAZAS}/load_SA_' || nombre_tabla_reducido || '_${FECHA_HORA}' || '.log ');
        UTL_FILE.put_line(fich_salida_load, 'echo "Inicia Proceso: `date +%d/%m/%Y\ %H:%M:%S`"  >> ${' || 'NGRD' || '_TRAZAS}/load_SA_' || nombre_tabla_reducido || '_${FECHA_HORA}' || '.log ');
        UTL_FILE.put_line(fich_salida_load, 'echo "Fecha de Carga: ${FCH_CARGA}"  >> ${' || 'NGRD' || '_TRAZAS}/load_SA_' || nombre_tabla_reducido || '_${FECHA_HORA}' || '.log ');
        UTL_FILE.put_line(fich_salida_load, 'echo "Fecha de Datos: ${FCH_DATOS}"  >> ${' || 'NGRD' || '_TRAZAS}/load_SA_' || nombre_tabla_reducido || '_${FECHA_HORA}' || '.log ');
        UTL_FILE.put_line(fich_salida_load, 'echo "Forzado: ${BAN_FORZADO}"  >> ${' || 'NGRD' || '_TRAZAS}/load_SA_' || nombre_tabla_reducido || '_${FECHA_HORA}' || '.log ');
        ...
        UTL_FILE.put_line(fich_salida_load, 'MTDT_PROCESO.NOMBRE_PROCESO = ''load_SA_' || nombre_tabla_reducido || '.sh'' AND \');
        ...
        UTL_FILE.put_line(fich_salida_load, '  ${SHELL_SMS} "${TELEFONOS_DWH}" "${SUBJECT}"');
        UTL_FILE.put_line(fich_salida_load, '  echo ${SUBJECT} >> ' || '${' || NAME_DM || '_TRAZAS}/' || 'load_SA' || '_' || nombre_tabla_reducido || '_${FECHA_HORA}.log');        
        UTL_FILE.put_line(fich_salida_load, '  echo `date` >> ' || '${' || NAME_DM || '_TRAZAS}/' || 'load_SA' || '_' || nombre_tabla_reducido || '_${FECHA_HORA}.log');
        UTL_FILE.put_line(fich_salida_load, '  exit 0');
        UTL_FILE.put_line(fich_salida_load, 'fi');
        ...
        UTL_FILE.put_line(fich_salida_load, 'echo "Inicio de la carga de dimension ' || reg_tabla.TABLE_NAME || '"' || ' >> ' || '${' || NAME_DM || '_TRAZAS}/' || 'load_SA' || '_' || nombre_tabla_reducido || '_${FECHA_HORA}.log');
        UTL_FILE.put_line(fich_salida_load, '');
        /* (20180110) Angel Ruiz.  Meto este workaround porque estos dos procesos necesitan otra cadena de conexion para poder terminar y no dar error de HEAP */
        if (reg_tabla.TABLE_NAME = 'SA_PARQUE_DESC_MES1' or reg_tabla.TABLE_NAME = 'SA_PARQUE_SVA_MES1') then
        ...
        UTL_FILE.put_line(fich_salida_load, '!quit');
        UTL_FILE.put_line(fich_salida_load, 'EOF');
        UTL_FILE.put_line(fich_salida_load, 'ERROR=`grep -ic -e ''Error: '' -e ''java.lang.RuntimeException'' -e ''Error: Could not open client transport'' -e ''Error: Error while'' -e ''ERROR jdbc.HiveConnection'' -e ''No current connection'' ${' || NAME_DM || '_TRAZAS}/' || 'load_SA' || '_' || nombre_tabla_reducido || '_${FECHA_HORA}.log`');
        UTL_FILE.put_line(fich_salida_load, '');
        ...
        UTL_FILE.put_line(fich_salida_load, '!connect ${CAD_CONEX_HIVE}/${ESQUEMA_ML}${PARAM_CONEX} ${BD_USER_HIVE} ${BD_CLAVE_HIVE}');
        UTL_FILE.put_line(fich_salida_load, 'select count(*) from ${ESQUEMA_ML}.' || reg_scenario.TABLE_NAME || ';');
        UTL_FILE.put_line(fich_salida_load, '!quit');
        UTL_FILE.put_line(fich_salida_load, 'EOF` >> '|| '${' || NAME_DM || '_TRAZAS}/' || 'load_SA' || '_' || nombre_tabla_reducido || '_${FECHA_HORA}.log ' || '2>&' || '1');
        ...
        UTL_FILE.put_line(fich_salida_load, 'fi');
        UTL_FILE.put_line(fich_salida_load, '');
        UTL_FILE.put_line(fich_salida_load, 'echo "La carga de la tabla ' || reg_tabla.TABLE_NAME || ' se ha realizado correctamente." >> ' || '${' || 'NGRD' || '_TRAZAS}/' || 'load_SA' || '_' || nombre_tabla_reducido || '_${FECHA_HORA}.log');
        UTL_FILE.put_line(fich_salida_load, 'exit 0');    
        UTL_FILE.put_line(fich_salida_load, '');    
