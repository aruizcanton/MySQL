...
BEGIN
  ...
  LOOP
    ...
    DBMS_OUTPUT.put_line('VALUES (' || reg.CVE_PROCESO || ', ''' || reg.NOMBRE_PROCESO || ''', ''' || reg.TIPO_PROCESO || ''', str_to_date(''' || to_char(reg.FCH_ALTA, 'DD/MM/YYYY') || ''', ''%d/%m/%Y''), ''' || reg.ESTADO || ''', str_to_date(''' || to_char(reg.FCH_ESTADO, 'DD/MM/YYYY') || ''', ''%d/%m/%Y''), str_to_date(''' || to_char(reg.FCH_REGISTRO, 'DD/MM/YYYY') || ''', ''%d/%m/%Y''), ''' || reg.ID_BLOQUE || ''', ''' || reg.PRECEDENCIA || ''', ''' || reg.DELAYED || ''' ); ' );
