...
BEGIN
  ...
  IF num_filas > 0 THEN
    ...
    LOOP
      ...
      DBMS_OUTPUT.put_line('GRANT select, insert, update, delete on ' || NAME_DM || '.' || r_mtdt_modelo_logico_TABLA.TABLE_NAME || ' TO '''|| OWNER_TC || '''@''%'';');
      DBMS_OUTPUT.put_line('GRANT select on ' || NAME_DM || '.' || r_mtdt_modelo_logico_TABLA.TABLE_NAME || ' TO '''|| OWNER_DWH || '''@''%'';');
      DBMS_OUTPUT.put_line('GRANT select on ' || NAME_DM || '.' || r_mtdt_modelo_logico_TABLA.TABLE_NAME || ' TO '''|| OWNER_RD || '''@''%'';');
      ...
      if (r_mtdt_modelo_logico_TABLA.CI = 'N') then
        ...
        DBMS_OUTPUT.put_line('GRANT select, insert, update, delete on ' || NAME_DM || '.T_' || nombre_tabla_reducido || ' TO '''|| OWNER_TC || '''@''%'';');
        DBMS_OUTPUT.put_line('GRANT select on ' || NAME_DM || '.T_' || nombre_tabla_reducido || ' TO '''|| OWNER_DWH || '''@''%'';');
        DBMS_OUTPUT.put_line('GRANT select on ' || NAME_DM || '.T_' || nombre_tabla_reducido || ' TO '''|| OWNER_RD || '''@''%'';');
      end if;      
      --DBMS_OUTPUT.put_line('');
